...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 1. Обработка отмены/изменения записи (не трогаем)
    ...
    # --- Новая логика консультаций по услугам ---
    state = user_data.get("state", "consult")
    form = user_data.get("form", {})

    if state == "consult":
        # Если пользователь хочет узнать об услугах или задаёт вопросы о ценах, объясни красиво!
        if is_consult_intent(text) or match_service(text):
            # Генерируем персональный промпт для OpenAI
            services_text = []
            for i, s in enumerate(SERVICES, 1):
            ...
            system_prompt = (
                "Ты — вежливый, доброжелательный и заботливый администратор стоматологической клиники. "
                "Отвечай подробно и понятно на любые вопросы клиента по услугам из списка. "
                "Объясняй всё простым, человеческим языком, предлагай выбрать услугу или проконсультироваться подробнее, но не навязывай запись. "
                "Приводи примеры и кратко объясняй разницу между услугами, если клиент спрашивает о сравнении. "
                "Не переходи к этапу записи, если пользователь не просит об этом явно.\n"
                f"Вот услуги клиники:\n{services_prompt}"
            )
            # Формируем историю только из последних 10 сообщений
            ...
            try:
                ...
                reply = "Извините, не удалось получить ответ от ассистента 🤖"
